...
- 本地/导入时间线（Diagnostics Replay）：`src/components/diagnostics/DiagnosticsTimelineExplorer.jsx` 将 logs/errors/events 聚合为 breadcrumbs 时间线（类型筛选、关键词检索、展开详情、下载筛选结果），并支持一键定位到对应浏览器
