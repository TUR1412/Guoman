...
- **前端框架:** React 19
- **构建工具:** Vite 7
- **路由:** React Router 7（`HashRouter`，GitHub Pages 友好）
...
- **组件工作台:** Storybook 10（React-Vite，`npm run storybook`）
...
- **动效预设规范:** Route/Page/Modal/Toast 等动效优先复用 `src/motion/presets.ts`；reduced motion 判定统一使用 `src/motion/useAppReducedMotion.js`。
