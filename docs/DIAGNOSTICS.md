...
- **导入时间线**：将 logs/errors/events 聚合为按时间倒序的 breadcrumbs 时间线，支持类型筛选与关键词检索（更快还原行为链路）
...
导入诊断包后，诊断页还会提供「导入时间线」卡片：将 logs/errors/events 聚合为按时间倒序的 breadcrumbs 视图，支持类型筛选与关键词检索，适合快速还原“某次点击 → 某次报错”的前后文。
