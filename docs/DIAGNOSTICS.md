...
- **导入回放浏览**：导入成功后可直接浏览导入包中的 logs/errors/events（筛选/检索/展开/下载筛选结果），无需复制 JSON 手动检索
- **本地时间线**：将本地 logs/errors/events 聚合为按时间倒序的 breadcrumbs 时间线，支持类型筛选与关键词检索（更快还原行为链路）
- **导入时间线**：导入诊断包后，将导入包内 logs/errors/events 聚合为同样的 breadcrumbs 时间线，用于对照回放
- **时间线一键定位**：在时间线条目展开后，可一键定位到对应日志/错误/事件浏览器，并自动填充推荐筛选条件（更快深挖细节）
- **日志/错误/事件浏览器**：支持关键词检索、日志级别筛选（日志）、事件名筛选（事件）、展开查看详情（context/stack/payload），并可下载筛选结果
...
- `events`：本地埋点事件（local-first），用于还原关键交互路径与动作序列

对应构建逻辑位于：`src/utils/diagnosticsBundle.js`。

> 说明：为避免文件过大，导出的 `logs`/`errors`/`events` 默认会做数量上限裁剪（可在 `buildDiagnosticsBundle({ maxLogs, maxErrors, maxEvents })` 中调整）。
...
导入成功后，诊断页会显示导入包摘要，并提供“导入日志/错误/事件浏览器”用于直接检索与定位线索。

### 日志/错误/事件浏览器（Explorer）

诊断页提供“日志浏览器 / 错误浏览器 / 事件浏览器”，用于更快定位线索：

- **关键词**：同时匹配日志 `message/source/context`、错误 `message/source/stack`、事件 `name/payload`
- **级别筛选**（日志）：按 `ERROR/WARN/INFO/DEBUG` 过滤
- **事件名筛选**（事件）：按 `name` 精确过滤（便于回放某条行为链路）
- **展开查看**：每条记录支持展开查看详情（context/stack/payload）
- **下载筛选结果**：将当前筛选结果导出为 JSON 文件，便于分享与留档（local-first）

诊断页提供「本地时间线」卡片：将本地 logs/errors/events 聚合为按时间倒序的 breadcrumbs 视图，支持类型筛选与关键词检索，适合快速还原“某次点击 → 某次报错”的前后文。

导入诊断包后，诊断页还会提供「导入时间线」卡片：将导入包内 logs/errors/events 聚合为同样的 breadcrumbs 视图，用于对照回放与跨版本排障（不出网）。

> 小技巧：展开时间线条目后可直接点击“定位到日志/错误/事件浏览器”，系统会滚动到目标卡片并自动填充筛选条件，减少手动切换成本。
